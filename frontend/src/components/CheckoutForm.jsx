...
const CheckoutForm = ({ shippingMethod, setShippingMethod }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!stripe || !elements) {
            if (paymentMethod === 'card') return; // Stripe not ready
        }
        ...
        try {
            ...
            if (paymentMethod === 'card') {
                    ...
                    confirmParams: {
                        payment_method_data: {
                            billing_details: {
                                ...
                                address: {
                                    city: formData.city,
                                    country: 'PK', // Stripe expects 2-letter code
                ...
                if (paymentIntent && paymentIntent.status === 'succeeded') {
                    // Payment successful, proceed to place order in backend
                    await checkout({
            ...
            console.error(err);
            setError(err.message || err.response?.data?.message || 'Checkout failed. Please try again.');
        } finally {
